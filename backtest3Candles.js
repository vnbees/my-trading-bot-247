...
async function main() {
  ...
  const { trades, endingEquity, maxConcurrentTrades } = runSimulation(candles, config);
...
/**
 * Kiểm tra xem một nến có phải là nến tăng không
 */
...
/**
 * Kiểm tra xem một nến có phải là nến giảm không
 */
...
/**
 * Kiểm tra xem có 3 nến tăng liên tiếp không
 */
function hasThreeConsecutiveBullish(candles, index) {
...
/**
 * Kiểm tra xem có 3 nến giảm liên tiếp không
 */
function hasThreeConsecutiveBearish(candles, index) {
  if (index < 2) return false;
  const c1 = candles[index - 2];
  const c2 = candles[index - 1];
  const c3 = candles[index];
...
/**
 * Tính Stop Loss cho lệnh long: giá thấp nhất của 3 nến
 */
function calculateStopLossLong(candles, index, config) {
  if (index < 2) return null;
  const c1 = candles[index - 2];
  const c2 = candles[index - 1];
  const c3 = candles[index];
  const minLow = Math.min(c1.low, c2.low, c3.low);
  
  if (config.priceTick > 0) {
    return Number(roundToTick(minLow, config.priceTick).toFixed(config.priceDecimals));
  }
  return Number(minLow.toFixed(config.priceDecimals));
}

/**
 * Tính Stop Loss cho lệnh short: giá cao nhất của 3 nến
 */
function calculateStopLossShort(candles, index, config) {
  if (index < 2) return null;
  const c1 = candles[index - 2];
  const c2 = candles[index - 1];
  const c3 = candles[index];
  const maxHigh = Math.max(c1.high, c2.high, c3.high);
  
  if (config.priceTick > 0) {
    return Number(roundToTick(maxHigh, config.priceTick).toFixed(config.priceDecimals));
  }
  return Number(maxHigh.toFixed(config.priceDecimals));
}

/**
 * Tính Take Profit dựa trên entry price, stop loss và risk:reward ratio
 */
...
function runSimulation(candles, config) {
  const startIndex = 2; // Cần ít nhất 3 nến để kiểm tra
  ...
  for (let i = startIndex; i < candles.length - 1; i++) {
    ...
    // Kiểm tra 3 nến tăng liên tiếp -> Long
    if (hasThreeConsecutiveBullish(candles, i)) {
      direction = 'long';
      stopLoss = calculateStopLossLong(candles, i, config);
    }
    // Kiểm tra 3 nến giảm liên tiếp -> Short
    else if (hasThreeConsecutiveBearish(candles, i)) {
      direction = 'short';
      stopLoss = calculateStopLossShort(candles, i, config);
    }

    if (direction && stopLoss !== null) {
      ...
      if (entryIndex < candles.length) {
        ...
        if (lotSizing) {
          ...
          if (config.verbose) {
            console.log(`[BACKTEST] Mở lệnh ${direction.toUpperCase()} tại nến ${entryIndex}: Entry=${entryPrice}, SL=${stopLoss}, TP=${takeProfit}`);
          }
...
function renderSummary(trades, endingEquity, config, maxConcurrentTrades) {
  ...
  trades.slice(-5).forEach((trade) => {
    ...
    console.log(
      `  • ${trade.direction.toUpperCase()} | ${trade.exitReason} | Entry=${formatNumberOrDash(trade.entryPrice)} @ ${entryLocal} → Exit=${formatNumberOrDash(trade.exitPrice)} @ ${exitLocal} | PnL=${formatNumber(trade.pnl)} (${trade.pnlPercent.toFixed(2)}%) | Fee=${feeLog}`
    );
  });
}
